...
withinClass_scatter = np.linalg.inv(withinClass_scatter)
r = np.dot(withinClass_scatter,betweenClassScatter)
...
for i in range(2):
    x = min(list_evalues)
    lis_minimum.append(list_evalues.index(x))
    list_evalues.pop(list_evalues.index(x))
res = []
res.append(eigen_vectors[:, lis_minimum[0]])
res.append(eigen_vectors[:, lis_minimum[1]])
